...
$(function () {
    ...
    function renderWebglTable(webgl_implementation, supported_webgl_implementations) {
        ...
        if (webgl_impl) {
            ...
            const M = webgl_params.length, N = 2;


            //const WEBGL_SERVICE_ARRAY = new Array(M);

            // for(var ind = 0; i < M; ++i){
            //     WEBGL_SERVICE_ARRAY[ind] = new Array(N);
            // }
            ...
            for (let webgl_param_index in webgl_params) {
                ...
                // localStorage.setItem("BEST_FLOAT_PRECISIONS",webgl_impl.gl.getShaderPrecisionFormat(webgl_impl.gl.FRAGMENT_SHADER));

                // if(webgl_params[webgl_param_index] == "VERSION"){
                //     localStorage.setItem("Ver", render_context.getParameter(render_context[webgl_params[webgl_param_index]]));
                // }
                if (webgl_params[webgl_param_index] instanceof Array) {
                    ...
                    for (let webgl_param_subindex in webgl_params[webgl_param_index]){
                        ...
                        let par_val = render_context.getParameter(
                            render_context[webgl_params[webgl_param_index][webgl_param_subindex]]);
                        if(webgl_params[webgl_param_index][webgl_param_subindex] == "RED_BITS"){
                            localStorage.setItem("RED_BITS",par_val);
                        }
                        else if(webgl_params[webgl_param_index][webgl_param_subindex] == "GREEN_BITS"){
                            localStorage.setItem("GREEN_BITS",par_val);
                        }
                        else if(webgl_params[webgl_param_index][webgl_param_subindex] == "BLUE_BITS"){
                            localStorage.setItem("BLUE_BITS",par_val);
                        }
                        else if(webgl_params[webgl_param_index][webgl_param_subindex] == "ALPHA_BITS"){
                            localStorage.setItem("ALPHA_BITS",par_val);
                        }
                        else if(webgl_params[webgl_param_index][webgl_param_subindex] == "STENCIL_BITS"){
                            localStorage.setItem("STENCIL_BITS",par_val);
                        }
                        else if(webgl_params[webgl_param_index][webgl_param_subindex] == "DEPTH_BITS"){
                            localStorage.setItem("DEPTH_BITS",par_val);
                        }
                        webgl_param_value += par_val;
                ...
                // localStorage.setItem(webgl_params[webgl_param_index], webgl_param_value);
                ...
                if(DEBUG){
                    // WEBGL_SERVICE_ARRAY[webgl_param_index][0] = webgl_params[webgl_param_index];
                    // WEBGL_SERVICE_ARRAY[webgl_param_index][1] = webgl_param_value;

                    console.log(webgl_params[webgl_param_index] + " = " + webgl_param_value);
                }

            }
            localStorage.setItem("MAX_TEXTURE_MAX_ANISOTROPY",getAnisotropy(webgl_impl.gl));
            localStorage.setItem("PRECISIONS",getFloatIntPrecision(webgl_impl.gl));
            // alert(render_context.getShaderPrecisionFormat(render_context.FRAGMENT_SHADER));


            // $.ajax({
            //     type: 'POST',
            //     url: '/api/url?arr='+WEBGL_SERVICE_ARRAY,
            //     error : function() {
            //         console.log("error");
            //     },
            //     success: function () {
